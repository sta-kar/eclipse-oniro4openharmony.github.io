...
If an emulator becomes unresponsive or gets into a broken state (e.g. an "Unable to find BMS Service" error), a cold boot or wiping its data is usually the fastest fix.
...
If the device isn't detected, unstable USB power management is a common culprit on Windows — try disabling power management for the USB Root Hub in Device Manager (Windows' own, not DevEco Studio's).
...
* **App fails to install on the emulator** — check for a `compileSdkVersion` mismatch with the target device, or a permission-level (ACL) issue in your signing profile.
