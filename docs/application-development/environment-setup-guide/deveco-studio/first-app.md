...
Two errors are common enough to call out specifically (both also covered in [Common Issues and Solutions](../common-issues.md)):
...
With your first app created, previewed, run, debugged, and packaged, head to [Emulator](emulator.md) if you haven't already set one up, back to [Workflow](workflow.md) for a closer look at the IDE itself, or to [Common Issues and Solutions](../common-issues.md) if something went wrong along the way.
